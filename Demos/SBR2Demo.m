function SBR2Demo(SourceSwitch);
%SBR2Demo(SourceSwitch);
%
%  SBR2Demo(SourceSwitch) demonstrates the polynomial EVD approximated by 
%  the function SBR2() for different type of parahermitian matrices. The
%  input parameter SourceSwitch can be selected as:
%
%  SBR2Demo('Random') produces an abritrary complex valued 5x5 polynomial 
%  parahermitian matrix of order 10.
%
%  SBR2Demo('Simple1') decomposes a 3x3 real valued polynomial parahermitian 
%  matrix R(z) of order 4:
%                | 1     -.4*z       0     |
%       R(z) =   | -.4*z   1     .5*z^{-2} |
%                | 0     .5*z^2      3     |     
%
%  SBR2Demo('Simple2') decomposes a 5x5 complex valued polynomial 
...
% S. Weiss, University of Southampton, 30/8/2005

if nargin==0,
   SourceSwitch='Random';
end;

disp('2nd Order Sequential Best Rotation (SBR2) Demo');
disp('----------------------------------------------');

%--------------------------------------
%  Define Scenario
%--------------------------------------
% obtain example parahermitian matrix
...
% decomposition
[H,Gamma] = SBR2(R,maxiter,epsilon,mu,'SBR2');
